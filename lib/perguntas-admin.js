...
/** Admin e supervisor podem gerir perguntas via API (validação na rota; RLS no Supabase deve alinhar). */
export function assertPerfilGestaoPerguntas(perfil) {
  const p = String(perfil ?? "").trim();
  if (p !== "admin" && p !== "supervisor") {
    const err = new Error("Acesso restrito a administradores ou supervisores.");
    err.status = 403;
    throw err;
  }
}
...
export function validarPayloadPergunta(body, { isCreate = false } = {}) {
  ...
  const payload = {
    ...
    permite_nao_consta: !!body?.permite_nao_consta,
...
export function perguntaParaForm(p) {
  return {
    ...
    permite_nao_consta: !!p.permite_nao_consta,
    ativo: p.ativo !== false,
